...
for(let i = 1; i <= 100; i++){
    ...
    if(i % 7 === 0 && i > 30){
        console.log("\nPrimeiro múltiplo de 7 acima de 30: ", i);
        break
    }
};
...
console.log("Consoantes exibidas: ", quant);
...
console.log("Aluno A: ", verificar(notasAlunoA));
console.log("\nAluno B: ", verificar(notasAlunoB));
...
while(true){
    let palpite = dados.question("Digite seu palpite: ")
    tentivas++
...
console.log(`\nVocê precisou de ${tentivas} tentativa(s).`);
...
while(true){
    let num = dados.questionInt("Digite um numero (0 para sair): ")
    if(num === 0)break
     if(num < 0){
        console.log("\nIgnorando número negativo.");
        continue
}
soma += num
quantidade++
...
let media = total / qntVendas

console.log(`
\nVendas válidas: ${qntVendas}
Total arrecadado: R$ ${total}
Ticket médio: R$ ${media.toFixed(2)}`);
...
for(let i = 1; i <= 50; i++){
    ...
    soma0 += i
    console.log(`Somando: ${i} | Acumulado: ${soma0}`);
    
    if(soma0 > 200){
        console.log(`\nLaço encerrado em i = ${i} | Soma final: ${soma0}`);
        break
    }
};
