...
// E)
console.log(`
    | Sistema ativo: ${sistemaAtivo ? "Ativo" : "Não esta ativo"} 
    | Modo manutenção ativo: ${modoManutencao ? "Sim" : "Não"} 
    | O lugar esta vago: ${lugarVago ? "Vago" : "Não"} 
    | O lugar esta ocupado: ${lugarOcupado ? "Ocupado" : "Não esta ocupado"}`);
console.log("_______________________________");
...
// A)
let usuario1 = {
    peso: informacoes.questionFloat('Informe seu peso: '),
    altura: informacoes.questionFloat("Informe seu altura: ")
};
// B)
usuario1.imc = usuario1.peso / (usuario1.altura ** 2)
console.log(usuario1.imc.toFixed(2));
// C)
let imcAbaixoDoPeso = usuario1.imc < 18.5
// D)
let imcNormal = usuario1.imc >= 18.5 && usuario1.imc <= 24.9
// E)
let imcAcimaDoPeso = usuario1.imc >= 25 && usuario1.imc <= 29.9
// F)
console.log(`| IMC abaixo do peso: ${imcAbaixoDoPeso ? "Abaixo do peso"  : "Não esta abaixo do peso"}`);
console.log(`| IMC normal: ${imcNormal ? "Esta normal" : "Não esta normal"}`)
console.log(`| Esta acima do peso: ${imcAcimaDoPeso ? "Esta acima" : "Não esta acima"}`)
console.log("_______________________________");
...
console.log(`
    ...
    Esta reprovado: ${reprovado ? "Sim" : "não"}
    Precida de Ajuda: ${precisaDeAjuda ? "Sim" : "não"}`);
